...
by the `preprocess-tiles` and `generate-splits` scripts.

It takes a config and a seed as input, finds the corresponding split files
(e.g., `train_seed_42.txt`), and runs the training loop.
"""
import argparse
...
def load_tile_ids_from_file(split_path: Path) -> list[str]:
    ...
    if not split_path.exists():
        # DEV: Возвращаем пустой список, а не ошибку. `train.py` должен уметь
        # работать без валидационного сета. Ошибку выбросим позже, если train пуст.
...
def main():
    ...
    # DEV: Сид из командной строки имеет высший приоритет.
    # Это делает каждый запуск явным и независимым от того, что написано в YAML.
    set_seed(args.seed)
    ...
    train_tile_ids = load_tile_ids_from_file(splits_dir / "train.txt")
    val_tile_ids = load_tile_ids_from_file(splits_dir / "val.txt")

    if not train_tile_ids:
        raise ValueError(f"Training set is empty. Check if splits were generated correctly in {splits_dir}")
    ...
    dl_val = DataLoader(ds_val, batch_size=cfg['eval']['batch_size'], shuffle=False, num_workers=cfg['train']['num_workers'], pin_memory=True) if ds_val else None
    print(f"Data loaded: {len(ds_train)} train tiles, {len(ds_val) if ds_val else 0} validation tiles.")

    # --- 2. Model, Optimizer, Loss ---
    model = build_model(cfg).to(device)
    
    # Handle fine-tuning from an initial checkpoint
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        # Логируем и конфиг, и сид из командной строки
        ...
        for epoch in range(cfg['train']['epochs']):
            ...
            if avg_val_dice > best_val_dice:
                ...
                checkpoint_path = Path("./best_model.pt") # Temporary local path
